...
public class SongServiceImpl implements SongService {
    ...
    // =========================
    // GET SONG BY ID (CACHE ENABLED)
    // =========================
    @Override
    @Cacheable(value = "songs", key = "#songId")
    public SongResponseDTO getSongById(Long songId) {

        System.out.println("🔥 DB HIT: fetching song from database");
    ...
    // =========================
    @Override
    @CacheEvict(value = "songs", key = "#songId")
    ...
    public Page<SongResponseDTO> searchSongs(String query, String genre, String sort, int page, int size) {
        ...
        Sort sorting = "playCount".equalsIgnoreCase(sort)
        ...
        String normalizedGenre = (genre == null || genre.trim().isEmpty())
                ? null : genre.trim().toLowerCase();
        ...
        Specification<Song> spec = (root, cq, cb) -> {
            ...
            if (normalizedQuery != null) {
                predicates.add(cb.or(
                        cb.like(cb.lower(root.get("title")), "%" + normalizedQuery + "%"),
                        cb.like(cb.lower(root.get("artistProfile").get("displayName")),
                                "%" + normalizedQuery + "%")
                ));
            }
